...
public abstract class FrameDecoder extends SimpleChannelUpstreamHandler implements LifeCycleAwareChannelHandler {
    ...
    //累计缓冲区，就是为了保存decoder()没有处理完的buffer
    ...
    public void messageReceived(
        ...
        //判断m是不是ChannelBuffer,不是的话直接ctx.sendUpstream(e)传递给下一个handler
        //FrameDecoder只能处理ChannelBuffer类型的数据
        ...
        //cumulation 累计缓冲区，就是为了保存：
        // 1.decode()返回null(数据不完成，还需要等待)时没有处理的buffer
        // 2.decode()()没有处理完的buffer
        if (cumulation == null) {
            try {
    ...
    private void callDecode(
        ...
        while (cumulation.readable()) {
            ...
            //调用decode,就是我们自己实现的decode()方法
            Object frame = decode(context, channel, cumulation);
            if (frame == null) {
                //根本没有读取数据的情况(decode返回null的情况)，直接跳出返回，然后将input缓存到cumulation
            ...
            //这里就是decode()后符合结构的消息，
            //在这里调用Channels.fireMessageReceivedfireMessageReceived#ctx.sendUpstream继续传递给下一个handler处理
