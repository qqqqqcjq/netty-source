...
public class NioSocketMutilHanderThread {
    ...
    public void listenSelector() throws  IOException{
        ...
        while (true) {
            ...
            while(iteratorKey.hasNext()){
                ...
                //remove方法只是标记当前的元素要删除，但必须等到hander(selectionKey)处理完成后。这个元素还有人在用，也就是还有其他引用在，就不会删除。
                //如果重新用迭代器遍历，还是会遍历到的。连接就绪第二次处理会抛空指针，tcp是一对一的，只能连接1次；read就绪多次处理则会导致重复读，netty中就解决了这个问题。
                iteratorKey.remove();
    ...
    public static void main(String[] args) throws IOException {
        NioSocketMutilHanderThread nioSocketDemo = new NioSocketMutilHanderThread();
